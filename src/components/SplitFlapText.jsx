...
const SplitFlapText = ({
  ...
  useEffect(() => {
    ...
    // To trigger the initial flip on mount, we can initialize currentTextRef to empty spaces
    // and run animateTo(firstPhrase) once.
    currentTextRef.current = normalizePhrase('', width);
    setTiles(createTiles(currentTextRef.current));
    const initialAnimDuration = animateTo(firstPhrase);
    ...
    scheduleNext(safeCycleDelay + initialAnimDuration);
